...
def head_with_redirects(url: str, timeout: float = 10.0, max_redirects: int = 10) -> Tuple[Optional[int], Optional[str], List[str], Optional[str]]:
    """
    HEAD 요청으로 상태코드/최종URL/리다이렉션 체인/비고(note)를 반환.
    인증서 오류 발생 시 verify=False 로 재시도하며 insecure 표시.
    """
    ...
    try:
        ...
        return r.status_code, r.url, chain, None
    except requests.exceptions.SSLError as e:
        try:
            r = requests.head(url, allow_redirects=True, timeout=timeout, headers=headers, verify=False)
            chain = [h.headers.get("Location", "") for h in r.history if h.is_redirect]
            return r.status_code, r.url, chain, f"insecure (verify=False) due to: {e}"
        except Exception as e2:
            return None, None, chain, f"HTTP head error: {e} / fallback: {e2}"
    except Exception as e:
        return None, None, chain, f"HTTP head error: {e}"
...
def main():
    ...
    for row in all_rows:
        ...
        # HEAD 요청 (리다이렉션/상태)
        code, final_url, chain, http_note = head_with_redirects(norm_url)
